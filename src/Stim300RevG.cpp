...
:Stim300Base(MAX_PACKET_SIZE)
{
    ...
    this->inertial_values.aux = std::numeric_limits<double>::quiet_NaN();

    /** Revision G has 9 temperature sensors **/
...
int Stim300RevG::extractPacket(const uint8_t* buffer, size_t buffer_size) const
{
...
//  	    printf("%X \n", buffer[i]);
    ...
    if (this->modes == NORMAL)
    {
        ...
        //{
            ...
            /** Convert auxiliary raw to calibrated voltage**/
            this->inertial_values.aux = convertAux2Voltage(reinterpret_cast<uint8_t*>(&(*this->currentP).aux));
...
void Stim300RevG::printInfo()
{
    ...
    printf("Aux Status: %X\n", this->currentP->aux_status);
    std::cout<<"Aux[V]: "<<this->inertial_values.aux<<"\n";
...
double Stim300RevG::convertAux2Voltage(const uint8_t* buffer)
{
    double aux_double = std::numeric_limits<double>::quiet_NaN();
    uint8_t copy_buffer[3];

    double twopower24 = 16777216;//2^24

    struct sensor_value *aux_value;
...
//  	printf ("Converting %X %X %X\n", buffer[0], buffer[1], buffer[2]);
        ...
        aux_value = reinterpret_cast<struct sensor_value *>(copy_buffer);

        aux_double = (double)(aux_value->value) / twopower24;

// 	std::cout<<"acc_double: "<<acc_double<<"\n";

    }

    return aux_double * 5;//units are in Voltage [V]

}

double Stim300RevG::convertAcc2Acceleration(const uint8_t* buffer)
{
...
bool Stim300RevG::verifyChecksum(boost::uint32_t &expintCRC, boost::uint32_t &calintCRC)
{
